...
const npmCacheRoot = join(packageRoot, ".tmp", "npm-cache");
...
async function main() {
    const tempRoot = await mkdtemp(join(tmpdir(), "html-core-consumer-"));
    let tarballPath;
...
async function packPackage() {
    const output = await run("npm", ["pack", "--json"], {
        cwd: packageRoot,
        capture: true,
    });
    ...
    if (!Array.isArray(entries) || entries.length !== 1) {
        throw new Error(
            `Expected one npm pack result, received ${Array.isArray(entries) ? entries.length : 0}.`,
        );
    }
...
async function runPackageBuild() {
    ...
    try {
        ...
        throw new Error(
            `tsup CLI not found at ${tsupCliPath}. Run pnpm install before consumer validation.`,
        );
    }
...
async function writeConsumerProject(consumerRoot, dependencyPath) {
    await writeJson(join(consumerRoot, "package.json"), {
        ...
        type: "module",
        dependencies: {
            "@ravenhill/html-core": `file:${dependencyPath}`,
        },
    });
    ...
    await writeFile(
        join(consumerRoot, "runtime-consumer.mjs"),
        runtimeConsumerSource,
    );
    await writeFile(
        join(consumerRoot, "subpath-runtime-consumer.mjs"),
        subpathRuntimeConsumerSource,
    );
    await writeFile(join(consumerRoot, "type-consumer.ts"), typeConsumerSource);
    await writeFile(
        join(consumerRoot, "subpath-type-consumer.ts"),
        subpathTypeConsumerSource,
    );
}
...
async function runTypeScriptCheck(consumerRoot) {
    ...
    try {
        ...
        throw new Error(
            `TypeScript binary not found at ${tscPath}. Run pnpm install before consumer validation.`,
        );
    }
...
function assertOutsideRepository(directory) {
    ...
    if (
        ...
        throw new Error(
            `Temporary consumer must be outside the repository: ${directory}`,
        );
    }
}
    ...
    return new Promise((resolvePromise, reject) => {
        ...
        child.on("close", (code) => {
            ...
            const captured = [stdout, stderr]
                .filter((output) => capture && output.trim().length > 0)
                .join("\n");
            const details = captured.length > 0 ? `\n${captured}` : "";
            reject(
                new Error(
                    `Command failed with exit code ${code}: ${command} ${commandArgs.join(" ")}${details}`,
                ),
            );
        });
    });
...
function spawnCommand(command, commandArgs, cwd, capture) {
    ...
    return spawn(
        ...
        [
            "/d",
            "/c",
            [executable, ...commandArgs.map(quoteShellArgument)].join(" "),
        ],
        {
            cwd,
            env,
...
if (HTML_CORE_PACKAGE_NAME !== "@ravenhill/html-core") {
    throw new Error("Unexpected package name.");
}

if (!/^\d+\.\d+\.\d+$/u.test(HTML_CORE_VERSION)) {
    throw new Error("Unexpected package version: " + HTML_CORE_VERSION);
}
`;

const subpathRuntimeConsumerSource = String.raw`const blockedSubpaths = [
    "@ravenhill/html-core/heading-level",
    "@ravenhill/html-core/src/index.js",
    "@ravenhill/html-core/dist/index.js",
];
...
const typeConsumerSource = String.raw`import type { HeadingLevel } from "@ravenhill/html-core";
...
import("@ravenhill/html-core/heading-level");
`;
