...
async function main() {
    const tempRoot = await mkdtemp(join(tmpdir(), "content-core-consumer-"));
    let tarballPath;
...
async function writeConsumerProject(consumerRoot, dependencyPath) {
    await writeJson(join(consumerRoot, "package.json"), {
        ...
        type: "module",
        dependencies: {
            "@ravenhill/content-core": `file:${dependencyPath}`,
        },
    });
...
if (!/^\d+\.\d+\.\d+$/u.test(CONTENT_CORE_VERSION)) {
...
if (LessonHref.create("notes/example").value !== "/notes/example/") {
    throw new Error("LessonHref did not normalize the lesson href.");
}
...
const lookup: LessonMetadataLookupResult = {
    kind: "missing",
    href: LessonHref.create("/notes/missing/"),
};

const resolution: LessonMetadataResolutionResult = {
    kind: "invalid",
    href: LessonHref.create("/notes/broken/"),
    issues: [issue],
};
