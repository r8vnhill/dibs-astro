...
2. **Post-build asset copy**: `scripts/copy-assets.mjs` copies 1,521 files to `dist/` after tsup (one SVG per source SVG)
3. **Pattern-based validation**: `assert-pack-files.mjs` uses SVG count parity rather than enumeration (scales to any icon count)
