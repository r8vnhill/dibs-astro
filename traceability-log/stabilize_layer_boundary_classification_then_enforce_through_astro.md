...
Refactor and harden the layer-boundary checker internals first, then move the primary enforcement path from the manual
`check:architecture` script into an Astro integration.

The pure checker remains in `scripts/lib`. The direct `check:architecture` command stays available for debugging, but
`pnpm build` and `pnpm check` become the main enforcement paths through Astro.

## Guiding Decisions

1. **Stabilize before relocating enforcement.** Clean up classification and rule-evaluation behaviour before wiring the
   checker into Astro’s lifecycle.

2. **Preserve public checker behaviour.** Exported classifier APIs, rule semantics, finding formats, and return shapes
   stay stable.

3. **Keep the checker pure.** Astro owns _when_ the checker runs; `scripts/lib` owns _what_ the checker does.

4. **Avoid dev-server noise.** Normal `astro dev` must not run layer-boundary checks.

5. **Keep a direct diagnostic path.** `pnpm run check:architecture` remains available even after Astro becomes the main
   enforcement path.

6. **No new dependency for this work.** Use existing Vitest coverage, plain data tables, injected test doubles, and
...
- Import-specifier helpers should live outside `layer-boundary-paths.mjs`; keep path normalization and import
...
scripts/__tests__/layer-boundary/layer-boundary-classification.test.ts
scripts/__tests__/layer-boundary/layer-boundary-rule-evaluation.test.ts
scripts/__tests__/layer-boundary/layer-boundary-checker.test.ts
...
Extend `scripts/__tests__/layer-boundary/layer-boundary-classification.test.ts`.
...
The uploaded cleanup plan already identifies these as the key classifier risks: overlap precedence, similarly named
paths, unresolved imports, unknown import kinds, and malformed/future records.
...
```bash
pnpm test:unit -- scripts/__tests__/layer-boundary/layer-boundary-classification.test.ts
```
...
Completed in `scripts/__tests__/layer-boundary/layer-boundary-rule-evaluation.test.ts`.
...
Extend `scripts/__tests__/layer-boundary/layer-boundary-rule-evaluation.test.ts`.
...
The cleanup plan explicitly calls out rule-evaluation coverage as the integration-level contract for
package-vs-resolved-target handling, exception matching, and fallback behaviour.
...
```bash
pnpm test:unit -- scripts/__tests__/layer-boundary/layer-boundary-rule-evaluation.test.ts
```
...
Centralize import-path extraction, import-kind classification, package-name extraction, and unresolved-import
classification.
...
Keep `layer-boundary-paths.mjs` focused on project path normalization. The cleanup plan specifically recommends a
separate import helper module rather than putting import-specifier logic into the path helper.
...
```bash
pnpm test:unit -- scripts/__tests__/layer-boundary/layer-boundary-classification.test.ts scripts/__tests__/layer-boundary/layer-boundary-rule-evaluation.test.ts
```
...
The integration plan requires exact string matching, a global skip variable, a gated config-setup check path, and a
run-once guard.
...
The Astro plan explicitly keeps `check:architecture` as a direct command while moving `pnpm build` and `pnpm check` onto
Astro lifecycle enforcement.
...
```bash
pnpm test:unit -- config/integrations/__tests__/layer-boundary-check.test.ts
pnpm test:unit -- scripts/__tests__/layer-boundary/layer-boundary-checker.test.ts
pnpm run check:architecture
pnpm run build
pnpm run check
```
...
This split keeps each commit reviewable and lets CI identify whether a failure comes from classifier semantics, rule
evaluation, Astro hook wiring, or package-script migration.
