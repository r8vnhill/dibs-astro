...
Phase 2 should extract fallback normalization into two small pure helpers and make both inline-field resolvers depend on
those helpers instead of maintaining local normalization paths.
...
- Keep production changes small enough that failures can be traced directly to normalization extraction rather than
  unrelated refactors.
...
Caller-facing render tests can remain deferred unless a unit-level failure suggests the helper extraction altered
effective rendering behavior.
...
Avoid adding property tests for URL handling unless they clearly improve confidence rather than duplicating
example-based cases.
