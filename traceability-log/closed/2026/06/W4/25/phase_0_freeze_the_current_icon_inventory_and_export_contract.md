...
`packages/astro-icons/scripts/test/icon-inventory.contract.test.mjs` and is wired into `test:audit-icons`
(28 tests passing total). It asserts the live `packages/astro-icons/src` corpus yields exactly 1,521 icons
(1,512 phosphor + 9 custom), that `file-csv.svg` stays classified as phosphor with export name `FileCsv`, that
`Bash`, `Python`, `NushellLogo`, and `FileCsv` are present exports, and that the committed artifact matches the
freshly generated bytes exactly. The frozen baseline now exists at `packages/astro-icons/migration/icon-inventory.json`,
generated via `audit-icons:update`. Running `audit-icons:update` a second time produced no diff, and `audit-icons`
succeeds against the committed artifact. Regenerating `packages/astro-icons/src/index.ts` produced no diff, confirming
no production asset or export changed. `pack:check` passed (1,526 files total, 1,521 SVGs in dist), confirming the
migration artifact is excluded from the published tarball via the existing `files` allowlist in `package.json`. Phase 0
is now complete.
