...
`INVALID_INVENTORY_COUNTS`). BDD-style coverage lives in `packages/astro-icons/scripts/test/icon-inventory.test.mjs` and
is wired into `test:audit-icons` (16 tests passing total).
...
`src/` and `migration/icon-inventory.json` relative to the script module (not `process.cwd()`), scans only direct `.svg`
entries non-recursively, serializes the frozen-artifact schema with two-space indentation and exactly one trailing line
feed, and compares `--check` against the complete committed bytes rather than reparsing JSON. CLI argument parsing
rejects missing mode, both modes together, and unknown arguments. BDD-style coverage with One Piece-themed temporary
fixtures lives in `packages/astro-icons/scripts/test/audit-icons.test.mjs` and is wired into `test:audit-icons` (24
tests passing total). `package.json` now also exposes `audit-icons` (`--check`) and `audit-icons:update` (`--write`).
Running `pnpm --filter @ravenhill/astro-icons audit-icons` against the real source correctly fails with a
missing-artifact diagnostic, since `migration/icon-inventory.json` has not been generated yet.

Cycle 4 is complete: the real corpus contract test lives in
`packages/astro-icons/scripts/test/icon-inventory.contract.test.mjs` and is wired into `test:audit-icons` (28 tests
passing total). It asserts the live `packages/astro-icons/src` corpus yields exactly 1,521 icons (1,512 phosphor + 9
custom), that `file-csv.svg` stays classified as phosphor with export name `FileCsv`, that `Bash`, `Python`,
`NushellLogo`, and `FileCsv` are present exports, and that the committed artifact matches the freshly generated bytes
exactly. The frozen baseline now exists at `packages/astro-icons/migration/icon-inventory.json`, generated via
`audit-icons:update`. Running `audit-icons:update` a second time produced no diff, and `audit-icons` succeeds against
the committed artifact. Regenerating `packages/astro-icons/src/index.ts` produced no diff, confirming no production
asset or export changed. `pack:check` passed (1,526 files total, 1,521 SVGs in dist), confirming the migration artifact
is excluded from the published tarball via the existing `files` allowlist in `package.json`. Phase 0 is now complete.
...
# Cycle 4 — Freeze and Verify the Repository Contract [DONE]
