...
/**
 * Test suite for {@link buildCommitUrl}.
 *
 * These tests verify that commit URLs are:
 *
 * - Canonically constructed for each supported platform.
 * - Correctly routed according to platform-specific URL conventions.
 * - Structurally valid for arbitrary repository names and commit hashes.
 *
 * The suite combines:
 *
 * - Deterministic example-based tests (DDT style).
...
describe.concurrent("buildCommitUrl", () => {
     ...
     *
     * GitLab format:
     *   https://gitlab.com/{user}/{repo}/-/commit/{hash}
     */
    test.each(
        [
            [
        ...
        "builds canonical commit URL for %s",
        (platform, ref, hash, expected) => {
     ...
     * - Commit hash
     *
     * The generated URL must:
     *
     * - Contain the exact commit hash.
     * - Use the correct platform-specific commit route.
     * - Avoid malformed patterns such as "//commit/".
     *
     * This protects against accidental route regressions.
     */
        ...
        fc.assert(
            fc.property(
                ...
                (platform, user, repo, hash) => {
                    ...
                    // Hash must always appear verbatim.
                    expect(url).toContain(hash);

                    // Avoid accidental double slash before commit segment.
                    expect(url.includes("//commit/")).toBe(false);

                    // Platform-specific route assertion.
                    if (platform === "github") {
                        expect(url).toContain(`/commit/${hash}`);
                    } else {
                        expect(url).toContain(`/-/commit/${hash}`);
                    }
                },
