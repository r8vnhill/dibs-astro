...
import { TAB_SYNC_EVENT_PREFIX, type TabsSyncEvent, type TabValue } from "./events";
...
export class TabsController {
    ...
    private initializeActiveTab(): void {
        const initial = (this.syncKey && localStorage.getItem(this.storageKey))
    ...
    private handleKeyNavigation = (e: KeyboardEvent): void => {
        ...
        if (dir === "start") {
            ...
            for (let i = 1; i < this.triggers.length; i++) {
                const next = (this.currentTabIndex + i * dir + this.triggers.length)
