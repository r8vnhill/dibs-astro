...
export const buildCatalogArtifactFromTurtle = (
    ...
    for (const quad of parser.parse(ttl)) {
        if (quad.graph.termType !== "DefaultGraph") {
            abortValidation(
                sourceLabel,
                "named graphs are not supported in the bibliography catalog.",
            );
        }
        if (quad.subject.termType !== "NamedNode") {
            abortValidation(
                sourceLabel,
                "blank node subjects are not supported in the bibliography catalog.",
            );
        }
        if (quad.object.termType === "BlankNode") {
            abortValidation(sourceLabel, `blank node object found on "${quad.subject.value}".`);
        }
    ...
    for (const record of Array.from(recordsByIri.values()).sort((a, b) => a.order - b.order)) {
        ...
        abortValidation(sourceLabel, `node "${record.id}" has unsupported type "${primaryType}".`);
    }
