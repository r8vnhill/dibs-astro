...
if (!user || user === "0" || user === 0 || user === "root") {
    throw new Error("OCI runtime is configured to run as root.");
}
...
for (const layer of manifest.layers) {
    const finding = tarEntries(await readBlob(registry, image, layer.digest)).find((entry) =>
        forbidden.some((item) => entry === item || entry.startsWith(`${item}/`))
    );
...
async function selectPlatformManifest(value, platform) {
    ...
    const descriptor = value.manifests.find((item) =>
