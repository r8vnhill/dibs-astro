...
if (project.license !== "BSD-2-Clause" || !license.includes("Redistribution and use in source and binary forms")) {
    throw new Error("Authoritative project license metadata is incomplete.");
}
...
const platform = root.manifests.find((item) =>
...
const attestations = root.manifests.filter((item) =>
...
console.log(
    `OCI provenance passed for ${metadata.image}@${metadata.digest}; ${attestations.length} attestation manifest(s) found.`,
);
...
function assertEqual(actual, expected, field) {
    if (actual !== expected) {
        throw new Error(`OCI ${field} metadata mismatch: expected ${expected}, received ${actual ?? "missing"}.`);
    }
}
